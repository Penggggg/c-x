<template>
  <div class="order-doctors">
    ...
    <div class="time-select">
      <div class="doctor-introduce" >
        <div class="avatar">
          <mu-avatar size="100">
            <img :src="currentDoctor.doctor.images">
          </mu-avatar>
        </div>
        <!-- <div class="content"> -->
          <div class="line-one">
            <p class="name">{{currentDoctor.doctor.name}}</p>
            <p class="price">挂号费：¥{{(currentDoctor.scheduleInfoList[0].regFee/100).toFixed(0)}}</p>
          </div>
          <div class="line-two">{{currentDoctor.doctor.deptName}} |  {{currentDoctor.doctor.doctorLevel | dicPos(dic)}}</div>
          <div class="line-three">{{currentDoctor.doctor.description}}</div>
        <!-- </div> -->
      </div>
      <div class="schedule">
        ...
        <!-- 弹出层 - 时段 -->
      <mu-bottom-sheet class="bottom-select" :key="2"
        :open.sync="showTime">
  ...
  <my-drawer :show.sync="showForm" title="预约确认" >
    <div >
      <!-- 基本信息 -->
      <div class="base-info" >
        <my-form-v2 ref="form"
            ...
            <p class="time tips"
             >
              <span v-if="formData.date">
                {{ formData.date }} {{ timeObj.hasOwnProperty('startTime')? timeObj.startTime : ''}}
              </span>
              <span v-else
                class="placeholder">
                预约时间
          ...
          <div class="multiinput-block">
            ...
            <mu-text-field :rows="3"
              ...
              full-width
              class="no-border"
              :placeholder="$router.history.current.query.deptId === '28c321b4bd594fc0b297cc52d8b1b8d5'? ' 请备注您具体选择的哪项评估？（运动心肺，FMS功能性动作筛查，健康体适能综合评估）':'1000字以内'"
...
Vue.filter("dicPos", function(pos,dic) {
  const res = dic.HMS_DOCTOR_POSITION.filter(item => { return item.value === pos; });
...
export default class BookingBodyCheck extends Vue {
  ...
  private handleDayChange(val) {
    ...
    val.events[0].customClass.indexOf('can-select')>=0 && this.loadTime();
  }
  ...
  private loadTime() {
    ...
    console.log( '时间表',currentDateList);
    Promise.all(
      ...
      allRes.map((item: any, index) => {
        ...
        this.timeOptions.push({
          value: String(index),
          disabled: state,
          name: !state ? `${item.startTime} - ${item.endTime}（ 余${item.regLeaveCount} ） ` : `${item.startTime} - ${item.endTime} ( 已满 )`,
          origin: item
        });
      });
      this.showTime = true;
  ...
  private check(): boolean {
    ...
    // 病情描述，对于运动门诊特定的校验需求
    if((this as any).$router.history.current.query.deptId === '28c321b4bd594fc0b297cc52d8b1b8d5' && !this.formData.desc){
      return err("请填写病情描述备注");
    }
    // 病情描述1000字
  ...
  /** 下一步 */
  private nextStep() {
    console.log('时间数据',this.calendarDate, this.timeObj,this.timeObj === {});
    if(!this.timeObj.hasOwnProperty('startTime')) {
      (this as any).$toast.error('请选择就诊时间');
      return;
    }
    this.showForm = true;
  }
  ...
  private fetchDic() {
    this.http$
      .get<any>(
        {
          url: `/api/common/dic?typeCode=HMS_COMM_SEX,HMS_ORDER_QUESTION_TYPE,HMS_ORDER_CONSULTATION_WAY,HMS_DOCTOR_POSITION`
        },
        {
          errMsg: "加载错误",
          loadMsg: "加载中..."
      ...
      .then(res => {
        ...
        this.dic = Object.assign({}, this.dic, {
          ...
          HMS_DOCTOR_POSITION: data.HMS_DOCTOR_POSITION.map(x => ({
            label: x.name,
            value: x.itemValue
          }))
        });
