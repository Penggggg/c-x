...
import { MouseContainer, MyForm } from '@cvte/ai-web-util/components';
...
export const TextLabelTag = ({ params, tags, canAction = true, defaultValue = [ ], onDelete, onUpdate, onCreate, onSure }: TextLabelTag ) => {
    ...
    useEffect(( ) => {
        ...
        const last = defaultRef.current;
        const isSame = defaultValue.every(( d, k ) => {
            if ( !last[ k ]) return false;
            const { paramKey, offset, length, tagId } = d;
            return paramKey === last[ k ].paramKey && offset === last[ k ].offset && length === last[ k ].length && tagId === last[ k ].tagId;
        })
        
        /** 判断值是否相同 */
        if ( !isSame ) {
            const defaultLabel = defaultValue.map( d => {
                const { paramKey, offset, length, tagId } = d;
                const tag: any = tags.find( x => x.id === tagId );
                return {
                    tag,
                    txt: {
                        paramKey,
